...
The platform has four courses. Courses 1 and 2 below are built (`working-with-coding-agents`, `prompt-engineering-for-engineers`); the rest is the shortlist of what to build next, chosen so that each course reuses the platform's strengths (a running example, tiny examples, Eve, rubric-graded exercises) and fills a gap the first two leave open.
...
Suggested build order for the rest: 5, 8, then 3, 4, 6, 7, 9, 10. Courses 5 and 8 reuse Pip's Plant Shop.
